...
假定从连接上读取的数据，经协议handler校验并解码出了一个完整的请求包，此时协议handler会创建一个协议匹配的session（如nrpc协议对应NRPCSession），创建的session用于跟踪一个请求的完整生命周期，如session记录了客户端请求、服务端响应、服务处理过程中的错误事件、分布式跟踪、日志信息等等。
